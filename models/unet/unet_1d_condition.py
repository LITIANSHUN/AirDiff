...
from diffusers.loaders import PeftAdapterMixin, UNet2DConditionLoadersMixin # lora; can 1d use this?
...
class UNet1DConditionModel(
    ...
    def __init__(
        ...
        # 确保down_block的层数和up_block的层数相同。
        # 确保每个down_block都有输出通道的说明。
        # 确保每个down_block里都有指示是否要使用self-attention。
        # 确保每个down_block都有注意力头的说明。
        # 确保每个down_block都有维度的说明。
        # 确保每个down_block都有层数的说明。
        # 因此这一步就是为了检查输入的合理性。

        # input #随后定义了一个单层卷积层，不改变图像尺寸，只改变图像通道。
        conv_in_padding = (conv_in_kernel - 1) // 2
        self.conv_in = nn.Conv1d(
            in_channels, block_out_channels[0], kernel_size=conv_in_kernel, padding=conv_in_padding
        )

        # time 接下来定义了时间编码投影层以及维度。这里实例化了一个self.time_proj类，可以对后续时间步进行处理。
        # 后续还实例化了一个self.time_embedding类，输入维度为self.time_proj的输出维度。
        ...
        # 随后对编码器隐藏状态做了一个投影，定义了一个投影层self.encoder_hid_proj，
        # 如果编码隐藏状态类型是“text proj”，那么该投影层是单层全连接层，
        # 如果类型是“text_image_proj”，那么该投影层是TextImagePeojection类，
        # 如果类型是“image_proj”，那么该投影层是ImageProjection类。
        # 不管该投影层是什么属性，反正最后的输出维度都是cross_attention_dim。
        ...
        # class embedding 接下来定义了一个类别编码层，
        # self.class_embedding，和上面的encoder隐藏状态一样，
        # 根据不同的输入类型有不同的结构，但总归输出的维度都是time_embed_dim。
        ...
        # 随后还是对输入参数的合理性做了一些处理，
        # 比如说有些参数在encoder或decoder的每个块都要用到，
        # 如果这个参数是列表，那就是合理的，如果这些参数是数字的话，
        # 也就说明每个块用到的参数是相同的，要把它们转换为列表形式。
        ...
        # 接下来定义了一个时间编码特征的维度，
        # 我猜测这里的维度应该就是输入到UNet每层里的时间编码的结构，
        # 如果类别特征与时间特征串联的话，那么维度拼接变为原来的2倍，
        # 如果不串联，那就是按位相加，维度还是保持不变。
        ...
        # count how many layers upsample the seqs
    ...
    def forward(
        ...
        Args:
            sample (`torch.Tensor`): sample指的是网络的输入，也就是加噪后的噪声图noise，
                The noisy input tensor with the following shape `(batch, channel, length)`.
            timestep (`torch.Tensor` or `float` or `int`): The number of timesteps to denoise an input.
            timestep是模型的加噪步数，也就是模型的深度，这里的timestep是一个标量，表示模型的深度。
            encoder_hidden_states (`torch.Tensor`):是模型的condition的编码，文生图中就指的是文字特征。
                The encoder hidden states with shape `(batch, sequence_length, feature_dim)`.
            class_labels (`torch.Tensor`, *optional*, defaults to `None`): class_labels可有可无，如果有的话就会和时间编码加在一起，或是concat
                Optional class labels for conditioning. Their embeddings will be summed with the timestep embeddings.
            timestep_cond: (`torch.Tensor`, *optional*, defaults to `None`):  timestep_cond指的是时间步的条件特征。
                Conditional embeddings for timestep. If provided, the embeddings will be summed with the samples passed
                through the `self.time_embedding` layer to obtain the timestep embeddings.
            attention_mask (`torch.Tensor`, *optional*, defaults to `None`): attention_mask用于掩蔽输入的condition。
            ...
            down_block_additional_residuals: (`tuple` of `torch.Tensor`, *optional*):是加在unet下采样block的特征
                A tuple of tensors that if specified are added to the residuals of down unet blocks.
            mid_block_additional_residual: (`torch.Tensor`, *optional*):是加在mid层的特征
        ...
        # 1. time    时间编码 T 首先获取时间编码
        t_emb = self.get_time_embed(sample=sample, timestep=timestep)
        emb = self.time_embedding(t_emb, timestep_cond)   # 这一步就是将时间编码和时间条件编码加在一起
        aug_emb = None

        class_emb = self.get_class_embed(sample=sample, class_labels=class_labels)
        if class_emb is not None:
            if self.config.class_embeddings_concat:
                emb = torch.cat([emb, class_emb], dim=-1) # emb = concat(t_emb, class_emb) 获取到最终嵌入变量 emb
            else:
                emb = emb + class_emb                     # emb = t_emb + class_emb 获取到最终嵌入变量 emb

        aug_emb = self.get_aug_embed( # 获取到aug_emb, 这个是用于加在unet下采样block的特征, 
